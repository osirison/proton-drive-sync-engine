...
- `Proton Drive's command line tool isn't installed` / `This app drives the official tool rather than talking to Proton directly. Install it once and setup will carry on. Detected Debian — other distributions are in the help.` / `sudo apt install proton-drive` / `Copy` / `Check again` / `Installation help`
  - **Dropped, not made conditional (DEVIATIONS §102, #218):** no distribution packages `proton-drive`, so `sudo apt install proton-drive` is gone from `copy.js` with no replacement command. The title and `Check again` ship unchanged; the body ships as `This app drives the official proton-drive tool rather than talking to Proton directly, and no Linux distribution packages it — install it yourself, following its own instructions, then sign in and check again. Detected Debian — that doesn't change what to do here.` (or the undetected form). `Copy` and `Installation help` remain as constants nothing renders — the drawn frame above still contains both, on a command box and a help button neither exists in the build.
