...
| Delete approval (guarded) | Guard on; indexed file removed on one side | Daemon withholds the delete, lists it under `pending`, and touches nothing until `approve` |
| Delete approval (opted out) | `.proton-sync.toml` with the direction `= false` in the subtree | The delete under that subtree applies without approval; the rest stay guarded |
...
| Control IPC | IC-01 and IC-02 are covered by the IPC integration test plus watcher regression tests. The delete-approval control flow (withhold → `pending` → `approve` → apply) is covered end-to-end across the daemon and control-CLI boundary by `tests/ipc_cli.rs::delete_approval_withholds_a_remote_delete_until_approved_over_ipc`, and the guard/inheritance/fail-safe/cursor-hold behaviors by `src/daemon.rs` gate tests and `src/dirconfig.rs` resolver tests | A live daemon pause/resume test with real Proton traffic remains pending; the delete-approval flow has not been exercised against a real account |
| Interruption safety | Failed side-effect handling verifies that actions completed before a later failure are checkpoint-committed while the failed action itself is never recorded and the pass does not count as a successful sync (no cursor advance) — ADR 0003 semantics. An IPC process test covers failed upload behavior across the daemon and control CLI boundary. A real `SIGINT` sent to the daemon process during a blocked upload is covered by a deterministic integration test (`tests/ipc_cli.rs::sigint_during_blocked_upload_exits_cleanly_without_partial_index_state`): it proves the daemon reaches a clean, tightly bounded shutdown (asserted well under the fake CLI's configured command timeout, not just eventually), commits no partial index state, and releases its lockfile | The test uses a fake CLI blocked on a sentinel file, not a real large upload against the live Proton Drive service; see the note below the table for why this is sufficient |
...
SIGINT note: the daemon's main loop runs its reconcile step via `tokio::task::block_in_place`, so a `SIGINT` is only *acted on* by that blocking call once its own polling loop notices a shared cancellation flag. A separate, always-running task sets that flag the instant the signal is delivered (independent of whatever the blocking call is doing), and `run_once`'s polling loop (`src/proton.rs`) checks the flag every `CANCELLATION_POLL_INTERVAL` (100ms), so in practice the stuck call is interrupted and its whole process group is killed within about one poll interval - not only once the call's own `CommandPolicy` timeout would otherwise have fired. The deterministic test asserts this tightened bound directly (elapsed time, not just eventual completion) and already proves the properties that matter for data safety: the daemon does not hang for the stuck call's full timeout window, no partial index state is committed (the blocked upload is that scenario's only action, so nothing had completed; under ADR 0003 checkpoint semantics, actions completed *before* an interruption would — correctly — remain committed), and the lockfile is released. A live, large-upload variant of this scenario would exercise the same code path and is not expected to behave differently; it has not been run live because it offers no additional coverage over the deterministic test while consuming real account quota and time.
