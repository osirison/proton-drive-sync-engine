...
| `daemon.rs` | The **runtime**. Reconciles once on startup, then runs a `select!` loop over filesystem-watch events, IPC connections, a periodic scan, an events-poll interval, and shutdown signals. Dispatches between full-tree bootstrap and event-driven incremental reconcile; funnels every side effect and the single post-success commit through one path. Holds the delete-approval gate and an advisory lockfile. |
