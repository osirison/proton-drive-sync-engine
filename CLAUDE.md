...
- **`scan_interval` is not a snapshot cadence in events mode; a degraded session is.** With events on and a live event source, a `scan_interval` tick is just another incremental (usually idle) pass — identical to an `EVENTS_POLL_INTERVAL` tick. It deliberately does **not** seed the resync floor to force a snapshot (#52): that would reinstate the periodic full walk the invariant above removes. The exception is a **degraded session** (`events_driven` on but no event source — locked keyring, headless host, no CLI session): every pass is then a full-tree snapshot, so the fast events-poll arm is gated on `event_source.is_some()` and those snapshots — plus the per-pass `reacquire_event_source_if_needed` retry — ride `scan_interval` instead of the 30s poll (#50). The daemon warns once at startup naming both intervals, keeping this cause distinguishable from the other "keeps doing full syncs" log lines (periodic resync / event-stream fallback / structural fallback). (Guards: `a_degraded_session_does_not_full_walk_on_the_events_poll_cadence`, `a_degraded_session_still_reconciles_on_the_scan_interval`.)
